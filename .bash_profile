...
#
function stopMcafee(){
	sudo launchctl list | grep mcafee |  awk '{print $3}' | xargs -L1 sudo launchctl remove
	echo "Searching for remaining mcafee processes"	
	sudo launchctl list | grep mcafee
}
function copyPlist(){
	cp /Users/mccjo26/Documents/Work/scripts/CAMobileAppAnalytics/josh_ios_camdo\ copy.plist .
}
function copyLibrary(){
	cp /Users/mccjo26/Documents/Work/scripts/CAMobileAppAnalytics .
}
...
# looks at current directory for a certain file pattern regex. IFFY regexes btw
...
function gcb(){
	git branch --list | egrep "$@" | xargs git checkout
}
...
function copyCPItem(){
find . -regex '.*\/"$@".pdf' -exec cp "{}"  graph/ps1/ \;
}
